...
def htc_command(command, filename, htc_file, job_name, folder="./", cwd="./", hours="200"):
    command_string = " ".join([x for x in command if x])

    fp = open(os.path.join(folder, htc_file), 'w+')
    fp.write("UNIVERSE = vanilla\n")
    fp.write("SHOULD_TRANSFER_FILES = Yes\n")
    fp.write("WHEN_TO_TRANSFER_OUTPUT = ON_EXIT_OR_EVICT\n")
    fp.write("TRANSFER_INPUT_FILES = ems\n")
    fp.write("TRANSFER_OUTPUT_FILES = ems\n")
    fp.write("OUTPUT = ex1.out\n")
    fp.write("ERROR = ex1.error\n") 
    fp.write("LOG = ex1.log [%s]\n"%command_string)
    fp.write("QUEUE\n")
    fp.close()
