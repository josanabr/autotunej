...
Usage:
  run.py [--input=INPUT] [--dry] [--htcondor] [--pbs] [--quiet] [-q, --queue]
...
import shutil
import itertools
from processruns.runner import permute_runs, q_command, run_command, pbs_command, htc_command
...
for r in runs:
    ...
    if args['--dry']:
        ...
        htcondor_file = 'submit.htcondor'
        job_name = "%s_%s" % (r['options']['-d'], r['options']['-p'])
        
        if 'wallhours' not in r:
            sys.stderr.write("You must include `wallhours` in your `--input` when using HTCondor submit.\n");
            continue

        pid = htc_command(command, r['filename'], htcondor_file, job_name=job_name,
                folder=folder, cwd=folder, hours=r['wallhours'])
    elif args['--queue']:
        job_name = "%s_%s" % (r['options']['-d'], r['options']['-p'])
